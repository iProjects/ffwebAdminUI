    ...
    public partial class AddTransactionTypeForm : Form
    {
        ...
        private void AddTransactionTypeForm_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                chkStatementFlag.DataSource = _DrCommCalcMethods;
                chkStatementFlag.ValueMember = "Key";
                chkStatementFlag.DisplayMember = "Value";
                chkStatementFlag.SelectedIndex = -1;
        ...
        private void btnAdd_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ...
            if (IsTransactionTypeValid())
            {
                try
                {
                    ...
                    txn.StatFlag = chkStatFlag.Checked.ToString();
                    ...
                    if (chkStatementFlag.SelectedIndex != -1)
                    {
                        txn.DrCommCalcMethod = chkStatementFlag.SelectedValue.ToString();
                    }
                    ...
                    if (tc.GetAllTransactionTypes().Any(i => i.ShortCode == txn.ShortCode))
                    {
                        MessageBox.Show("Short Code [ " + txn.ShortCode + " ] Exists!", "Fanikiwa", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                    if (!tc.GetAllTransactionTypes().Any(i => i.ShortCode == txn.ShortCode))
                    {
                        tc.CreateTransactionType(txn);
                    }
